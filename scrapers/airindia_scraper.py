"""
Air India Web Scraper (AI)

Scrapes real-time fares from Air India official website.
Handles:
...
class AirIndiaScraper:
    """Playwright scraper for Air India flight results."""
    ...
    def _search_url(self) -> str:
        return (
            f"https://www.airindia.in/en/book-a-flight.html?"
            f"origin={self.origin}&destination={self.destination}"
            f"&departdate={self.departure_date.strftime('%d%m%Y')}&paxcount=1"
        )
    
    def _extract_offers(self, page) -> list[FareQuote]:
        """Extract flight offers from Air India search results."""
        quote_rows: list[FareQuote] = []
        seen_keys: set[tuple[str, int]] = set()
        
        # Air India specific selectors
        flight_cards = page.locator(
            "[class*='flight'], [class*='result'], [data-testid*='flight']"
        ).all()
            ...
            try:
                ...
                quote_rows.append(
                    FareQuote(
                        ...
                        carrier=carrier,
                        source_site="airindia",
                        scrape_timestamp=datetime.now(timezone.utc),
                        departure_date=self.departure_date,
                        advance_purchase_days=days_until_departure,
                        fare_class="ECONOMY",
                        base_fare=float(total_fare * 0.75),
                        taxes_fees=float(total_fare * 0.25),
    ...
    def run(self) -> list[FareQuote]:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                # Fill search form
                try:
                    page.locator("input[placeholder*='From'], input[aria-label*='From']").first.fill(self.origin)
                    page.locator("input[placeholder*='To'], input[aria-label*='To']").first.fill(self.destination)
                except Exception:
                    pass
                
                # Search
                try:
                    page.locator("button:has-text('Search'), button:has-text('Find Flights')").first.click(
